    ...
    class FileSearchCommand
    {
        ...
        public void Content(string k)
        {
            ...
            var filesWithAllKeywords = new DirectoryInfo(baseDir).EnumerateFiles($"*.{extensionName}", SearchOption.AllDirectories)
                                  ...
                                  .Where(t => keywords.All(k => t.Item2.ToLowerInvariant().Contains(k.ToLowerInvariant())));
            ...
            IEnumerable<string> linesWithAnyKeywords = filesWithAllKeywords
                                                       ...
                                                       .Where(t => keywords.Any(k => t.line.ToLowerInvariant().Contains(k.ToLowerInvariant()))))
                                                       .OrderBy(t=> t.FullName)
                                                       .Select(t => $"{t.FullName}|{t.line.Trim()}");

            foreach(var line in linesWithAnyKeywords)
            {
